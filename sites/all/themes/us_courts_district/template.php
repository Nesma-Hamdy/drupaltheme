<?php
function us_courts_district_preprocess_html(&$vars) {
  ...
  if ($vars['is_front']){
	  $vars['head_title'] = implode(' | ', array(variable_get('site_name'), theme_get_setting('court_title')));
	  }
	  else {
  	  $vars['head_title'] = implode(' | ', array(drupal_get_title(), variable_get('site_name'), theme_get_setting('court_title')));
  	  }
}

function us_courts_district_preprocess_page(&$vars) {
 ...
 //Court Title
 $vars['court_title'] = theme_get_setting('court_title');
 ...
 foreach ($add_welcome_btns as $id => $btn) {
  if (isset($btn) && $btn != '') {
   $btn = us_courts_district_format_wb_vars($btn);
 ...
 //Basic Grid 12 Col Grid, 20px Gutter, 60px Col width
	if (!empty($vars['page']['sidebar_first']) && empty($vars['page']['sidebar_second'])) {
		$vars['sb_first_grid_classes'] = 'grid_3 pull_9';
		$vars['content_grid_classes'] = 'grid_9 push_3';
	}
	if (empty($vars['page']['sidebar_first']) && !empty($vars['page']['sidebar_second'])) {
		$vars['content_grid_classes'] = 'grid_9';
		$vars['sb_second_grid_classes'] = 'grid_3';
	}
	if (!empty($vars['page']['sidebar_first']) && !empty($vars['page']['sidebar_second'])) {
		$vars['sb_first_grid_classes'] = 'grid_3 pull_6';
		$vars['sb_second_grid_classes'] = 'grid_3';
		$vars['content_grid_classes'] = 'grid_6 push_3';
	}
	if (empty($vars['page']['sidebar_first']) && empty($vars['page']['sidebar_second'])) {
...
/*
  ...
  //  Remove page titles from Court - Court Locations pages

	if ( isset($vars['node']) && $vars['node']->type == 'court' ) {
	  $vars['title'] = t('Court Locations');
	}
...
function us_courts_district_menu_tree($vars) {
  return '<ul class="menu clearfix">' . $vars['tree'] . '</ul>';
}

function us_courts_district_form_alter(&$form, &$form_state, $form_id) {
...
function us_courts_district_preprocess_views_view(&$vars) {
 if($vars['name'] == 'faqs' && $vars['display_id'] == 'page') {
  us_courts_district_views_field_title_subs($vars['view'], 'field_faq_type');
 }
 if($vars['name'] == 'all_forms' && $vars['display_id'] == 'all_forms') {
  us_courts_district_views_field_title_subs($vars['view'], 'field_form_cat');
 }
}

function us_courts_district_views_field_title_subs($view, $field_name) {
 ...
 */
function us_courts_district_format_wb_vars($vars) {
